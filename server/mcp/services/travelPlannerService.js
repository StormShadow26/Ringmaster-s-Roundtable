...
import { estimateBudget, summarizeBudgetText } from './budgetService.js';
...
export async function planTripBasedOnWeather(city, weatherData) {
  try {
    ...
    // Compute budget estimates based on the generated travel plan
    const budget = await estimateBudget(travelPlan, { travelers: 2, currency: 'USD' });
    console.log('🧾 Travel plan budget summary:', {
      city: travelPlan.destination.city,
      days: travelPlan.tripDuration.totalDays,
      cheap: budget.tiers.cheap.tripTotal,
      moderate: budget.tiers.moderate.tripTotal,
      luxury: budget.tiers.luxury.tripTotal
    });
    const budgetSummary = summarizeBudgetText(budget);
    travelPlan.budget = budget;
    travelPlan.budgetSummary = budgetSummary;
