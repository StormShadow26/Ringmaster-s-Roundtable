...
import { estimateBudget, summarizeBudgetText } from './services/budgetService.js';
...
// --- Register estimateBudgetForTravelPlan tool ---
registerTool(
  ...
  async ({ travelPlan, travelers, currency }) => {
    // Accept either object or JSON string
    const planObj = typeof travelPlan === 'string' ? JSON.parse(travelPlan) : travelPlan;
    const budget = await estimateBudget(planObj, { travelers, currency });
    console.log('📤 MCP tool estimateBudgetForTravelPlan returning:', {
      destination: planObj?.destination?.city,
      travelers,
      currency,
      cheap: budget.tiers.cheap.tripTotal,
      moderate: budget.tiers.moderate.tripTotal,
      luxury: budget.tiers.luxury.tripTotal
    });
    const text = summarizeBudgetText(budget);
    return {
      content: [
